...
sr2 = StrictRedis(host='localhost', port=6379, db=1)
app = Flask(__name__)
...
@app.route('/api/historyAnnotation', methods=['POST'])
# @cross_origin(origin='localhost',headers=['Content- Type','Authorization'])
def history_annotation():
    # To return logged predictions upon loggin
    response_object = {'status': 'success'}
    ...
    print("[/historyAnnotation] session end request received:", filename)
    # app.logger.info("[/submit] session end request received: "+str(data))
    sessionId = sr2.get(filename)
    if sessionId:
        response_object['found'] = 1
        response_object['sessionId'] = sessionId
    else:
        response_object['found'] = 1 ## TO CHANGE to 0
